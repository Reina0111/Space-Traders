...
function setTableOfShips() {
    let table = <HTMLTableElement>document.getElementById('Spaceships_table');
...
function setTableOfPlanets() {
    let table = <HTMLTableElement>document.getElementById('Platets_table');
...
function openShipInSpace(ship) {
    ...
    let table = <HTMLTableElement>document.getElementById('shipStorageInSpace');
    clear(table);
...
function refreshShipStorageOnPlanet(ship: string) {
    ...
    if(!ships[ship].items_quantity) ships[ship].items_quantity = 0;

    document.getElementById('storage_size').textContent = ships[ship].items_quantity + "/" + ships[ship].cargo_hold_size.toString();

    let table = <HTMLTableElement>document.getElementById('shipStorageOnPlanet');
    clear(table);
    for (let item in ships[ship].items) {
        let newRow = table.insertRow();
...
function refreshPlanetStorage(planet: string) {
    let table = <HTMLTableElement>document.getElementById('planetStorageForShip');
    clear(table);
...
function setTime() {
    ...
    let destination = (<HTMLSelectElement>document.getElementById('destination_list'))[index].textContent;
...
function openShipOnPlanet(ship: string) {
...
function openPlanet(planet: string) {
    ...
    let table = <HTMLTableElement>document.getElementById('planetStorage');
    clear(table);
    for (let item in planets[planet].available_items) {
        let newRow = table.insertRow();
    ...
    let list = <HTMLOListElement>document.getElementById('listOfShipsOnPlanet');
    clear(list);
    for (let ship in ships) {
...
function sellItems() {
    ...
    for (let i = 0; i < items.length; i++) {
        ...
        let soldQuantity = (<HTMLInputElement>items[i].children[3].childNodes[0]).value;
...
function buyItems() {
    ...
    for (let i = 0; i < items.length; i++) {
        ...
        let itemQuantity = items[i].children[1].textContent;
        let itemPrice = items[i].children[2].textContent;
        let buyedQuantity = (<HTMLInputElement>items[i].children[3].childNodes[0]).value;
